...
### 기획-코드-테스트 검증 (절대 불변)
- 작업 시작 전 구현 계획서의 **구현/테스트 케이스 레지스트리**와 A/E/X Case ID를 확인한다.
- Case ID가 없는 계획서, 구현 파일/단위가 불명확한 계획서, 테스트 매핑이 없는 계획서는 구현하지 않고 Orchestrator에 BLOCKED 보고한다.
- 이슈 본문에는 계획서 경로와 Case ID별 구현/테스트 예정 표를 채운다.
- PR 본문에는 Case ID별 실제 구현 파일/단위, 테스트 파일/테스트명, 검증 결과를 채운다.
- Case ID 확인표가 비었거나 계획서의 Case ID를 누락한 PR은 완료로 선언하지 않는다.
- UI 변경 Case는 스크린샷/접근성/반응형 검증 근거를 Case ID에 연결한다.
...
- [ ] 구현 계획서 경로와 Case ID(A/E/X) 확인표를 이슈/PR에 채웠는가?
- [ ] 모든 계획서 Case ID가 구현 파일/단위와 테스트 파일/테스트명에 연결되어 있는가?
- [ ] TDD 사이클을 따랐는가?
- [ ] `npm run lint` 에러 없는가?
